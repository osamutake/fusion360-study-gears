...
- クラウンギアの溝形状生成時に幅指定が誤っていたのを直した
- 円柱ギアおよびクラウンギアの生成を少し高速化した
- 移動されたコンポーネントの下に歯車を作る際の座標計算にミスがあったのを直した
- かさ歯車の `printable` 指定時に45度の傾きが逆側に取られてしまう問題を直した

## 20250917

- はすば歯車の精度向上のため断面生成数を増やした
- コンポーネントの下にウォームホイールを生成した際にエラーが出ないよう修正
- かさ歯車に `printable` チェックボックスを追加し３Ｄプリンタで印刷しやすい形状を出力できるようにした
